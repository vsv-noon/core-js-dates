...
function getDayName(date) {
  ...
  const longDayName = d.toLocaleString('en-US', {
    weekday: 'long',
    timeZone: 'UTC',
  });
  return longDayName;
...
function getNextFriday(date) {
  const dateOfNextFriday = new Date(date.getTime());
  const currentDayOfWeek = dateOfNextFriday.getDay();
  const FRIDAY = 5;
  let daysUntilFriday = FRIDAY - currentDayOfWeek;

  if (daysUntilFriday <= 0) {
    daysUntilFriday += 7;
  }

  dateOfNextFriday.setDate(dateOfNextFriday.getDate() + daysUntilFriday);

  return dateOfNextFriday;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const daysDiff = timeDiffMs / oneDayMs + 1;
...
function getWeekNumberByDate(date) {
  ...
  const weekNo = Math.ceil(((currentDate - yearStart) / msInDay + 1) / 7);
